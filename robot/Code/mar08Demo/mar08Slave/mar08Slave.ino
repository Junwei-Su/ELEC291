...
void receiveEvent(int howMany) {
  ...
  if ( Wire.available() == 1 ) {    // one byte means that user is currently selecting a mode
    ...
    if ( x != prevLCDCode ) {       // reduces flickering text on LCD if user does not press a button
      ...
      switch (x) {
        ...
        case 3:
          ...
          break;
        case 4:     // special case for bonus matrix processing
          lcd.print("Matrix Ready");
          lcd.setCursor(2, 1);
          lcd.write(byte(6));
  ...
  else if ( Wire.available() == 3 ) {   // 3 bytes means data is for line following
    ...
    byte empty = 5; // display an empty box for no detection on that sensor
    byte full = 4;  // full box for sensors that detect the line
    switch(boxCode) {
      case 0:
        lcd.write(full);
        lcd.setCursor(7, 1);
...
/*
   Used for reading the momentary switches
*/
void requestEvent() {
